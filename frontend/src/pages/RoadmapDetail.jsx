...
const RoadmapDetail = () => {
    ...
    const loadRoadmap = async () => {
        try {
            ...
            if (response.data.roadmap?.weeklyModules && Array.isArray(response.data.roadmap.weeklyModules)) {
                const currentWeek = response.data.roadmap.weeklyModules.find(
    ...
    const isWeekUnlocked = (week) => {
        ...
        return week.prerequisiteModules.every(prereqWeekNum => {
            const prereqWeek = roadmapData.roadmap?.weeklyModules?.find(w => w.weekNumber === prereqWeekNum);
            return prereqWeek && prereqWeek.status === 'completed';
        });
    };

    const getWeekProgress = (week) => {
        if (!week.dailyTasks || !Array.isArray(week.dailyTasks) || week.dailyTasks.length === 0) return 0;
    ...
    const getOverallProgress = () => {
        if (!roadmapData.roadmap || !roadmapData.roadmap.weeklyModules || !Array.isArray(roadmapData.roadmap.weeklyModules)) return 0;
        const completed = roadmapData.roadmap.weeklyModules.filter(w => w.status === 'completed').length;
        ...
        <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-purple-50 px-4 py-8">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="card">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                            ...
                            <div className="flex flex-col gap-2">
                                <span className={`px-4 py-2 rounded-lg font-medium text-center capitalize ${roadmap.status === 'completed'
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="p-4 bg-gray-50 rounded-xl">
                                ...
                                <p className="text-xl font-bold text-gray-900">{roadmap?.weeklyModules?.length || 0}</p>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-xl">
                                <div className="flex items-center gap-2 text-gray-600 mb-1">
                    ...
                    <div className="card mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {roadmap.milestones.map((milestone, idx) => (
                                <div
                                    key={idx}
                                    className={`p-4 border-2 rounded-xl transition-all ${milestone.completed
                ...
                <div className="space-y-4">
                    ...
                    {roadmap?.weeklyModules && Array.isArray(roadmap.weeklyModules) && roadmap.weeklyModules.map((week) => {
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-4 flex-1">
                                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold ${week.status === 'completed'
                                                ? 'bg-green-100 text-green-700'
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                                                    ...
                                                    <span>
                                                        {(week.dailyTasks && Array.isArray(week.dailyTasks) ? week.dailyTasks.filter(t => t.status === 'completed').length : 0) || 0} / {(week.dailyTasks && Array.isArray(week.dailyTasks) ? week.dailyTasks.length : 0) || 0} tasks
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="space-y-3">
                                                    {week.dailyTasks.map((task, taskIdx) => (
                                                        <div
                                                            key={taskIdx}
                                                            className={`p-4 border-2 rounded-lg transition-all ${task.status === 'completed'
                                                        ...
                                                        >
                                                            <div className="flex items-start gap-3">
                                                                ...
                                                                <div className="flex-1">
                                                                    <h5 className={`font-semibold mb-1 ${task.status === 'completed'
