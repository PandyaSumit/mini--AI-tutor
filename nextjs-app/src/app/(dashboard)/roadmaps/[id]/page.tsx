/**
 * Roadmap Detail Page
 * Shows detailed roadmap with weekly modules, tasks, and milestones
 */
...
export default function RoadmapDetailPage() {
  ...
  const loadRoadmap = async () => {
    try {
      ...
      if (response.roadmap?.weeklyModules && Array.isArray(response.roadmap.weeklyModules)) {
        const currentWeek = response.roadmap.weeklyModules.find(
          (w: any) => w.status === 'in_progress' || w.status === 'not_started'
        );
        if (currentWeek) {
  ...
  const handleTaskToggle = async (weekNumber: number, taskIndex: number, currentStatus: string) => {
    setUpdating(true);
    try {
  ...
  const isWeekUnlocked = (week: any) => {
    ...
    return week.prerequisiteModules.every((prereqWeekNum: number) => {
      const prereqWeek = roadmapData.roadmap?.weeklyModules?.find(
        (w: any) => w.weekNumber === prereqWeekNum
      );
  ...
  const getWeekProgress = (week: any) => {
    if (!week.dailyTasks || !Array.isArray(week.dailyTasks) || week.dailyTasks.length === 0)
      return 0;
    const completed = week.dailyTasks.filter((t: any) => t.status === 'completed').length;
  ...
  const getOverallProgress = () => {
    if (
      !roadmapData.roadmap ||
      !roadmapData.roadmap.weeklyModules ||
      !Array.isArray(roadmapData.roadmap.weeklyModules)
    )
      return 0;
    const completed = roadmapData.roadmap.weeklyModules.filter(
      (w: any) => w.status === 'completed'
    ).length;
    return Math.round((completed / roadmapData.roadmap.weeklyModules.length) * 100);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-gray-50 text-gray-700 font-medium rounded-lg border border-gray-200 transition-all mb-4"
          >
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              ...
              <div className="flex flex-col gap-2">
                <span
                  className={`px-4 py-2 rounded-lg font-medium text-center capitalize ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="p-4 bg-gray-50 rounded-xl">
                ...
                <p className="text-xl font-bold text-gray-900">
                  {roadmap.weeklyModules?.length || 0}
                </p>
              </div>
              <div className="p-4 bg-gray-50 rounded-xl">
                <div className="flex items-center gap-2 text-gray-600 mb-1">
                ...
                <p className="text-xl font-bold text-blue-600">{getOverallProgress()}%</p>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          {roadmap.weeklyModules &&
            Array.isArray(roadmap.weeklyModules) &&
            roadmap.weeklyModules.map((week: any) => {
              const isUnlocked = isWeekUnlocked(week);
              const isExpanded = expandedWeeks.includes(week.weekNumber);
              const progress = getWeekProgress(week);

              return (
                <div key={week.weekNumber} className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                  {/* Week Header */}
                  <button
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        >
                          {isUnlocked ? week.weekNumber : <Lock className="w-5 h-5" />}
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                            ...
                            <span>
                              {(week.dailyTasks && Array.isArray(week.dailyTasks)
                                ? week.dailyTasks.filter((t: any) => t.status === 'completed').length
                                : 0) || 0}{' '}
                              /{' '}
                              {(week.dailyTasks && Array.isArray(week.dailyTasks)
                  ...
                  {isExpanded && isUnlocked && (
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      ...
                      {/* Daily Tasks */}
                      {week.dailyTasks && Array.isArray(week.dailyTasks) && week.dailyTasks.length > 0 && (
                        <div>
                          <h4 className="font-semibold text-gray-900 mb-3">Daily Tasks</h4>
                          <div className="space-y-3">
                            {week.dailyTasks.map((task: any, taskIdx: number) => (
                              <div
                                key={taskIdx}
                                    ...
                                    {task.resources &&
                                      ...
                                      task.resources.length > 0 && (
                                        <div className="mt-3 space-y-2">
                                          {task.resources.map((resource: any, resIdx: number) => (
                                            <a
                                              key={resIdx}
                                              href={resource.url}
                                              target="_blank"
